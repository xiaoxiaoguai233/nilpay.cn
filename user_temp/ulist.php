		...
		columns: [
			{
				...
				formatter: function(value, row, index) {
					...
					return '<b>'+value+'</b><a href="javascript:showKey('+value+',\''+row.key+'\') " style="margin: 0 5px" >[查看密钥]</a><span onclick="editGroup('+value+','+row.gid+')" style="color:blue">'+groupname+'</span>';
				}
			},
			{
				field: 'money',
			...
			},
			{
				...
				title: '结算账号/姓名',
				formatter: function(value, row, index) {
					return row.account ? '<span onclick="inputInfo('+row.uid+')" title="点击修改结算账号">'+(value==2?'<font color="green">WX:</font>':'')+(value==3?'<font color="green">QQ:</font>':'')+row.account+'  ['+row.username+']</span>' : '<span onclick="inputInfo('+row.uid+')" title="点击修改结算账号">未设置</span>';
				}
			},
			{
				field: 'qq',
				title: '联系方式',
				formatter: function(value, row, index) {
					return (value ? '[QQ:'+(isMobile() ? '<a href="mqqwpa://im/chat?chat_type=wpa&version=1&src_type=web&web_src=oicqzone.com&uin='+value+'">'+value+'</a>]' : '<a href="tencent://message/?uin='+value+'&amp;Site=qq&amp;Menu=yes">'+value+'</a>]') : '')+' ['+(row.phone ? row.phone : row.email) + ']';
				}
			},
			{
				field: 'url',
				title: '域名/添加时间',
				formatter: function(value, row, index) {
					return (value ? '<a href="//' + value + '" target="_blank">[' + value + ']</a>' :'' ) + (pay_domain=='true' ? ' [<a href="./domain.php?uid='+row.uid+'" target="_blank" >域名</a>]':'')+'  '+ row.addtime;
				}
			},
			{
				field: 'status',
				title: '状态',
				formatter: function(value, row, index) {
					let html = '<div style="display:flex;" class="status_a">';
					if(value == '1'){
			...
			},
			{
				...
				title: '操作',
				formatter: function(value, row, index) {
					return '<div style="display:flex;" class="status_a">' +
					            '<a href="./uset.php?my=edit&uid='+row.uid+'" class="btn btn-xs btn-info">编辑</a>&nbsp;' +
					            '<a href="./sso.php?uid='+row.uid+'" target="_blank" class="btn btn-xs btn-success">登录</a>&nbsp;' +
					            '<a href="./uset.php?my=delete&uid='+row.uid+'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此商户吗？\');">删除</a>&nbsp;' + 
					            '<a href="./order.php?uid='+row.uid+'" target="_blank" class="btn btn-xs btn-default">订单</a>&nbsp;' + 
					            '<a href="./slist.php?uid='+row.uid+'" target="_blank" class="btn btn-xs btn-default">结算</a>&nbsp;' + 
					            '<a href="./record.php?column=uid&value='+row.uid+'" target="_blank" class="btn btn-xs btn-default">明细</a>' + 
					        '</div>';
				}
			},
		],
